...
class ProductTest < ActiveSupport::TestCase
  ...
  test "product price must be positive" do
    ...
    product.image.attach(fixture_file_upload("lorem.jpg", "image/jpeg"))
  ...
  def new_product(filename, content_type)
    file_attachment = fixture_file_upload("#{filename}", content_type)
    Product.new(title: "My Book Title", description: "yyy", price: 1).tap do |product|
      product.image.attach(
        file_attachment
        # io: File.open("test/fixtures/files/#{filename}"), filename: filename, content_type: content_type
      )
    end
  end
